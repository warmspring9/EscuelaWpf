...
{
    public class SearchService : Service<Teacher>
    {
        static HttpClient client = new HttpClient();
        public SearchService()
        {
            connectionString += "/mobilguardian";
        }
        ...
        public async Task<List<Teacher>> GetSearchedTeacher(string name)
        {
            List<Teacher> Items = new();
            try
            {
                var client = new RestClient(connectionString +"/getbynameteacher/" + name);
                client.Timeout = -1;
                var request = new RestRequest(Method.GET);
        ...
        public async Task<List<Student>> GetSearchedStudent(string name)
        {
            List<Student> Items = new();
            try
            {
                var client = new RestClient(connectionString + "/getbynamestudent/" + name);
                client.Timeout = -1;
                var request = new RestRequest(Method.GET);
        ...
        public async Task<List<Guardian>> GetSearchedGuardian(string name)
        {
            List<Guardian> Items = new();
            try
            {
                var client = new RestClient(connectionString + "/getbynameguardian/" + name);
                client.Timeout = -1;
                var request = new RestRequest(Method.GET);
