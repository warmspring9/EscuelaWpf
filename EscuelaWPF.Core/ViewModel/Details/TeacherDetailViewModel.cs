    ...
    public class TeacherDetailViewModel : BaseViewModel
    {
        ...
        */
        ...
        public TeacherDetailViewModel(Teacher teacher)
        {
            ...
            {
                OriginalText = teacher.Name,
                EditedText = teacher.Name,
                IsEditing = true,
                Label = "Nombre"
            };
            LastName = new TextEntryViewModel()
            {
                OriginalText = teacher.Last_name,
                EditedText = teacher.Last_name,
                IsEditing = true,
                Label = "Apellido"
            };
            Id = new TextEntryViewModel()
            {
                OriginalText = teacher.Id.ToString(),
                EditedText = teacher.Id.ToString(),
                IsEditing = true,
                Label = "Id"
            };
            PhoneNumber = new PhoneNumberEntryViewModel()
            {
                OriginalText = teacher.Phone_num,
                EditedText = teacher.Phone_num,
                IsEditing = true,
                Label = "Telefono"
            };
            Image = ConditionImage(teacher.Image);
        ...
        private void Delete()
        {
            IoC.TeacherService.Delete(Id.OriginalText);
        }
        ...
        private async void SaveAsync()
        {
            if (PhoneNumber.IsValid())
            {
                ...
                try
                {
                    IoC.TeacherService.Put(temp);
                    await IoC.UI.ShowMesssage(new MessageBoxViewModel
                    {
                        Title = "Exito!",
                        Message = "Se actualizo el profesor con exito",
                        OkText = "OK"
                    });
                    Edit();
                }
